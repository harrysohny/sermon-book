...
function ttsTextForParagraph(p){
  ...
  // 화면에는 그대로 표시하되 TTS에서는 괄호와 괄호 안 내용을 읽지 않음
  let prev='';
  while(prev!==text){
    prev=text;
    text=text.replace(/\([^()]*\)/g,' ');
...
/* ===== 2026-09-08 집중 읽기 경험 업그레이드 ===== */
const READING_POS_KEY='sermonReadingPositions';
let readingPositions=JSON.parse(localStorage.getItem(READING_POS_KEY)||'{}');
let readingSaveTimer=null;

function readingEnsureProgress(){
  if(document.getElementById('readingProgress')) return;
  const bar=document.createElement('div');
  bar.id='readingProgress'; bar.className='reading-progress';
  bar.setAttribute('aria-hidden','true');
  document.body.appendChild(bar);
}
function readingCurrentPercent(){
  const article=document.querySelector('#detail article');
  if(!article || document.getElementById('detail')?.hidden) return 0;
  ...
  return Math.max(0,Math.min(100,((window.scrollY-top)/range)*100));
}
function readingUpdateProgress(){
  const bar=document.getElementById('readingProgress'); if(!bar)return;
  const active=!document.getElementById('detail')?.hidden;
  bar.style.setProperty('--reading-progress',(active?readingCurrentPercent():0)+'%');
  bar.classList.toggle('active',active);
}
function readingSavePosition(){
  if(!cur || document.getElementById('detail')?.hidden)return;
  const pct=Math.round(readingCurrentPercent());
  readingPositions[String(cur)]={percent:pct,scrollY:Math.round(window.scrollY),updated:Date.now()};
  localStorage.setItem(READING_POS_KEY,JSON.stringify(readingPositions));
  readingEnhanceRecent();
}
function readingScheduleSave(){clearTimeout(readingSaveTimer);readingSaveTimer=setTimeout(readingSavePosition,180)}
function readingRestoreOffer(n){
  document.getElementById('resumeReading')?.remove();
  const saved=readingPositions[String(n)];
  if(!saved || saved.percent<5 || saved.percent>94)return;
  const body=document.getElementById('body'); if(!body)return;
  const b=document.createElement('button');
  b.id='resumeReading'; b.className='resume-reading'; b.type='button';
  b.innerHTML=`<span>지난번 읽던 곳</span><strong>${saved.percent}%에서 계속 읽기 →</strong>`;
  b.onclick=()=>{window.scrollTo({top:saved.scrollY||0,behavior:'smooth'});b.remove()};
  body.insertAdjacentElement('beforebegin',b);
}
function readingEnhanceRecent(){
  document.querySelectorAll('#recentList .recent-item').forEach(btn=>{
    const num=+(btn.querySelector('.recent-num')?.textContent||0);
    const saved=readingPositions[String(num)], badge=btn.querySelector('.read-badge');
    if(badge&&saved&&saved.percent>=5&&saved.percent<95){badge.textContent=saved.percent+'%';badge.title='마지막 읽은 위치'}
  });
}
function readingMakeTtsCollapsible(){
  const box=document.getElementById('ttsBox');
  if(!box||box.dataset.collapsible==='1')return;
  box.dataset.collapsible='1';
  const title=box.querySelector('.tts-title'); if(!title)return;
  title.setAttribute('role','button');title.setAttribute('tabindex','0');title.setAttribute('aria-expanded','false');
  [...box.children].filter(x=>x!==title).forEach(x=>x.classList.add('tts-collapsible-content'));
  box.classList.add('tts-collapsed');
  const toggle=()=>{const c=box.classList.toggle('tts-collapsed');title.setAttribute('aria-expanded',String(!c))};
  title.onclick=toggle;
  title.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();toggle()}};
}
function readingBackButton(){
  const b=document.querySelector('#detail > #back'); if(b)b.classList.toggle('compact',window.scrollY>360);
}
function readingEndMatter(){
  document.getElementById('archiveEndMatter')?.remove();
  const body=document.getElementById('body'), s=S.find(x=>x.number===cur); if(!body||!s)return;
  const end=document.createElement('section');end.id='archiveEndMatter';end.className='archive-end-matter';
  end.innerHTML=`<div class="archive-end-book">『시냇가에 심은 나무』</div><div class="archive-end-meta">설교 ${String(s.number).padStart(3,'0')} · ${esc(s.scripture||'')}<br>故 손상기 목사</div>`;
  const source=document.querySelector('#detail .source');
  if(source)source.insertAdjacentElement('beforebegin',end);else body.insertAdjacentElement('afterend',end);
}
function readingAfterOpen(n){
  readingEnsureProgress();readingMakeTtsCollapsible();readingRestoreOffer(n);readingEndMatter();readingBackButton();
  setTimeout(readingUpdateProgress,60);
}
;(()=>{
  const oldOpen=open;
  open=function(n){if(cur)readingSavePosition();oldOpen(n);setTimeout(()=>readingAfterOpen(Number(n)),0)};
  const oldHome=showHome;
  showHome=function(){readingSavePosition();oldHome();readingUpdateProgress();setTimeout(readingEnhanceRecent,0)};
})();
window.addEventListener('scroll',()=>{readingUpdateProgress();readingBackButton();readingScheduleSave()},{passive:true});
window.addEventListener('resize',readingUpdateProgress,{passive:true});
window.addEventListener('pagehide',readingSavePosition);
document.addEventListener('visibilitychange',()=>{if(document.hidden)readingSavePosition()});
setTimeout(()=>{readingEnsureProgress();readingEnhanceRecent()},300);
